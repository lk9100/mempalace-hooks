...
. "$HOOK_DIR/mempal_hook_common.sh"
...
if [ -f "$TRANSCRIPT_PATH" ]; then
    EXCHANGE_COUNT="$(mempal_count_human_messages "$TRANSCRIPT_PATH" 2>/dev/null)"
else
    EXCHANGE_COUNT=0
fi
...
if [ "$SINCE_LAST" -ge "$SAVE_INTERVAL" ] && [ "$EXCHANGE_COUNT" -gt 0 ]; then
    ...
    if is_valid_transcript_path "$TRANSCRIPT_PATH" && [ -f "$TRANSCRIPT_PATH" ]; then
        mempal_mine_active_transcript "$TRANSCRIPT_PATH" "$SESSION_ID" "$STATE_DIR/hook.log" sync
